...
void createCarSection(lv_obj_t* parent, const char* title, const char* car_name) {
    ...
    styleLabelPrimary(ui.car.car_label);
    positionAndAlign(ui.car.car_label, SCREEN_WIDTH-(4*PADDING)-16-120, 0, 120, LV_TEXT_ALIGN_RIGHT);
    ui.car.power_desc = lv_label_create(container);
    lv_label_set_text(ui.car.power_desc, "LEISTUNG");
    styleLabelSecondary(ui.car.power_desc);
    lv_obj_set_pos(ui.car.power_desc, 0, 18);
    ui.car.power_label = lv_label_create(container);
    lv_label_set_text(ui.car.power_label, "0W");
    styleLabelPrimary(ui.car.power_label);
    lv_obj_set_pos(ui.car.power_label, 0, 33);
    ui.car.charged_desc = lv_label_create(container);
    lv_label_set_text(ui.car.charged_desc, "GELADEN");
    styleLabelSecondary(ui.car.charged_desc);
    positionAndAlign(ui.car.charged_desc, (SCREEN_WIDTH - (2 * PADDING) - 16) / 2 - 45, 18, 90, LV_TEXT_ALIGN_CENTER);
    ui.car.duration_desc = lv_label_create(container);
    lv_label_set_text(ui.car.duration_desc, "RESTZEIT");
    styleLabelSecondary(ui.car.duration_desc);
    positionAndAlign(ui.car.duration_desc, SCREEN_WIDTH-(4*PADDING)-16-120, 18, 120, LV_TEXT_ALIGN_RIGHT);
    ui.car.charged_value = lv_label_create(container);
    lv_label_set_text(ui.car.charged_value, "0 kWh");
    styleLabelPrimary(ui.car.charged_value);
    positionAndAlign(ui.car.charged_value, (SCREEN_WIDTH - (2 * PADDING) - 16) / 2 - 45, 33, 90, LV_TEXT_ALIGN_CENTER);
    ui.car.ladedauer_value = lv_label_create(container);
    lv_label_set_text(ui.car.ladedauer_value, "--:--");
    styleLabelPrimary(ui.car.ladedauer_value);
    positionAndAlign(ui.car.ladedauer_value, SCREEN_WIDTH-(4*PADDING)-16-120, 33, 120, LV_TEXT_ALIGN_RIGHT);
    int phaseBarY = 52; int phaseBarWidth = 30; int phaseBarHeight = 4; int phaseBarSpacing = 2;
    for (int i = 0; i < 3; i++) {
    ...
    positionAndAlign(ui.car.plan_desc, (SCREEN_WIDTH - (2 * PADDING) - 16) / 2 - 60, 90, 120, LV_TEXT_ALIGN_CENTER);
    ...
    positionAndAlign(ui.car.plan_value, (SCREEN_WIDTH - (2 * PADDING) - 16) / 2 - 60, 110, 120, LV_TEXT_ALIGN_CENTER);
    ...
    positionAndAlign(ui.car.plan_soc_value, (SCREEN_WIDTH - (2 * PADDING) - 16) / 2 - 60, 130, 120, LV_TEXT_ALIGN_CENTER);
